...
- (void)viewDidLoad { // add player to the homescreen
	...
	if (!dimBlurViewHS && ([homescreenBlurAmountValue doubleValue] != 0.0 || [homescreenDimValue doubleValue] != 0.0)) {
		...
		if (![dimBlurViewHS isDescendantOfView:[self view]]) [[self view] insertSubview:dimBlurViewHS atIndex:1];
	}
	...
	if (!blurHS && [homescreenBlurAmountValue doubleValue] != 0.0) {
		...
		if (![blurViewHS isDescendantOfView:dimBlurViewHS]) [dimBlurViewHS addSubview:blurViewHS];
	}
	...
	if (!dimViewHS && [homescreenDimValue doubleValue] != 0.0) {
		...
		if (![dimViewHS isDescendantOfView:dimBlurViewHS]) [dimBlurViewHS addSubview:dimViewHS];
	}
...
- (void)viewDidLoad { // add player to the control center
	...
	if (!dimBlurViewCC && ([controlCenterBlurAmountValue doubleValue] != 0.0 || [controlCenterDimValue doubleValue] != 0.0)) {
		...
		if (![dimBlurViewCC isDescendantOfView:[self view]]) [[self view] insertSubview:dimBlurViewCC atIndex:2];
	}
	...
	if (!blurCC && [controlCenterBlurAmountValue doubleValue] != 0.0) {
		...
		if (![blurViewCC isDescendantOfView:dimBlurViewCC]) [dimBlurViewCC addSubview:blurViewCC];
	}
	...
	if (!dimViewCC && [controlCenterDimValue doubleValue] != 0.0) {
		...
		if (![dimViewCC isDescendantOfView:dimBlurViewCC]) [dimBlurViewCC addSubview:dimViewCC];
	}
